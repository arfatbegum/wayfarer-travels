...
const UpcomingPackages = () => {
    ...
    const UpcomingPackages = packages?.filter((tourPackage: any) => {
    ...
    return (
        <div className="container lg:p-10 p-4 mx-auto">
            ...
            <div className="grid lg:grid-cols-4 grid-cols-1 lg:gap-3 md:space-y-0 space-y-6">
                {UpcomingPackages && UpcomingPackages.length > 0 ? (
                    UpcomingPackages.slice(-4).map((tourPackage: any) => (
                        <PackageCard key={tourPackage.id} tourPackage={tourPackage} />
                    ))
                ) : (
                    <p>No upcoming packages available.</p>
                )}
            </div>
