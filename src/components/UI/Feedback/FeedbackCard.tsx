...
const FeedbackCard: React.FC<FeedbackProps> = ({ feedback }) => {
    return (
        <div className="flex flex-col">
            <div>
                <div className="group w-full bg-white relative flex flex-col items-center hover:bg-[#0f337a] cursor-pointer shadow-md md:p-12 p-6">
                    <div className="text-gray-600 group-hover:text-white flex flex-col items-center">
                        ...
                        <p className="xl:w-80 text-base leading-normal text-center mt-4">
                            <span className='mb-2 block'>{feedback?.comment}</span>
                            <span>{feedback?.suggestions}</span>
                        </p>
                    </div>
                    <div className="text-white group-hover:text-[#0f337a] absolute bottom-0 -mb-6">
                ...
                <div className="flex flex-col items-center justify-center mt-10">
                    ...
                    />
                    <p className="text-base font-semibold leading-4 my-2 text-gray-800">{feedback?.user?.name}</p>
                </div>
            </div>
        </div>
    );
